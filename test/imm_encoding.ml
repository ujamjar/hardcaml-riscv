...
let () = 
  ...
  for i=(-2048) to 2047 do
    myassert (dec (Utils.i_imm (Asm.addi ~rd:0 ~rs1:0) ~imm:i)) i;
  done;

  (* s-type; 12 bit store offset *)
  for i=(-2048) to 2047 do
    myassert (dec (Utils.s_imm (Asm.sw ~rs1:0 ~rs2:0) ~imm:i)) i;
  done;

  (* sb-type; 12 bit conditional branch offset, multiplied by 2 *)
  for i=(-2048) to 2047 do
    myassert (dec (Utils.b_imm (Asm.beq ~rs1:0 ~rs2:0) ~imm:(i*2))) (i*2);
  done;

  (* u-type; lui, auipc - loads upper 20 bits of dest *)
  for i=(-(1 lsl 19)) to (1 lsl 19)-1 do
    myassert (dec (Utils.u_imm (Asm.lui ~rd:0) ~imm:(i*4096))) (i*4096);
  done;

  (* uj-type; 20 bit constant for jal, multiplied by 2 *)
  for i=(-(1 lsl 19)) to (1 lsl 19)-1 do
    myassert (dec (Utils.j_imm (Asm.jal ~rd:0) ~imm:(i*2))) (i*2);
  done;
